...
class PID
{
  ...
  //commonly used functions **************************************************************************
    PID(float*, float*, float*,        // * constructor.  links the PID to the Feedback, Control, and 
    ...
    void SetControlLimits(float, float); // * clamps the control to a specific range. 0-255 by default, but
  ...
  //available but not commonly used functions ********************************************************
	...
	void SetControllerDirection(int);	  // * Sets the Direction, or "Action" of the controller. DIRECT
										  //   means the control will increase when error is positive. REVERSE
  ...
  private:
    ...
    float *myFeedback;              // * Pointers to the Feedback, Control, and Setpoint variables
    float *myControl;             //   This creates a hard link between the variables and the 
    float *mySetpoint;           //   PID, freeing the user from having to constantly tell us
                                  //   what these values are.  with pointers we'll just know.
			  
	unsigned long lastTime;
	float controlSum, lastFeedback;

	unsigned long SampleTime;
	float controlMin, controlMax;
	bool fbAuto, pOnE;
};
#endif
